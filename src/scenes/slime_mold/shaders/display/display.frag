...
void main() {
    ...
    avgIntensity = mix(intensity, avgIntensity, 0.1);

    vec3 color = palette(mod((avgIntensity + lastAgentDirection + uTime * 0.05 + (vUv.x + vUv.y) * 0.1) * 0.2, 1.0)) * (avgIntensity * (intensity * 0.8 + 0.1));
